...
def przegrales(punktyGracza, punktyBrukiera):
    ...
    if(punktyGracza>21):
        ...
        koniec = pygame.font.Font.render(pygame.font.SysFont("arial", 170), "oprzegrales", True,
                                     (0, 0, 0))
...
class Przycisk:
    def __init__(self, x_cord, y_cord, zdj, zdj1):
        self.x_cord = x_cord
        self.y_cord = y_cord
        self.przycisk_image = pygame.image.load(zdj)
        self.hovered_przycisk_image = pygame.image.load(zdj1)
    ...
    def draw(self, window):
        if self.hitbox.collidepoint(pygame.mouse.get_pos()):
            window.blit(self.hovered_przycisk_image, (self.x_cord, self.y_cord))
        else:
        ...
        for _ in range(ilosc_talii):
            for kolor in ['k', 'p', 't', 'k']:
                for wartosc in range(2, 14):
                    print(wartosc)
                    self.zdjecia.append((kolor, wartosc))

    def losuj_karte(self):
        karta = random.choice(self.zdjecia)
        self.zdjecia.remove(karta)
